...
async function goInProgressTask(){
    console.log("In Progress Liao")
    const urlStr=window.location.href;
    const url=new URL(urlStr);
    const taskId=url.searchParams.get("taskId");
    const inProgressTaskFB = firebase
    .functions()
    .httpsCallable('tasks-taskInProgress');
    let msg = '';
    await inProgressTaskFB({
        taskId:taskId
    }).then((result) => {
        msg = result.data;
        console.log(msg)
        ...
        const url=new URL(urlStr);
        const taskId=url.searchParams.get("taskId");
        window.location.replace( './taskInProgress.html?taskId='+taskId)
    });
    return msg;
}
